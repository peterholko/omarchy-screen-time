...
import "PatchQueue.js" as PatchQueue
...
Item {
  ...
  function patch(obj) {
    if (password === "") return
    pendingPatch = PatchQueue.merge(pendingPatch, obj)
    note = "Saving…"
    noteColor = fadeText(0.5)
    sendPendingPatch()
  }

  function sendPendingPatch() {
    if (patchProc.running || pendingPatch === null || password === "") return
    var next = pendingPatch
    pendingPatch = null
    patchProc.launched = false
    patchProc.pendingPassword = password
    patchProc.command = [root.clientPath, "--password-stdin", "config", "patch", JSON.stringify(next)]
    patchProc.running = true
  }

  function handlePatchReply(raw) {
    var payload
    try { payload = JSON.parse(raw) } catch (error) { payload = {} }
    patchProc.pendingPassword = ""
    if (payload.ok) {
      note = pendingPatch !== null ? "Saving…" : "Saved."
      noteColor = okColor
      Qt.callLater(sendPendingPatch)
    } else {
      pendingPatch = null
      note = payload.error === "bad_password" || payload.error === "password_locked_out"
        ? "Unlock the parent controls again to save changes."
  ...
  Process {
    ...
    stdout: StdioCollector { id: patchOutput; waitForEnd: true }
    onExited: root.handlePatchReply(patchOutput.text)
  ...
  ScrollView {
    ...
    contentHeight: content.implicitHeight + Style.space(40)
    ...
    FocusScope {
      ...
      Column {
        ...
        Grid {
          ...
          Repeater {
            ...
            delegate: SettingsNumberField {
              ...
              to: 1440
              stepSize: 5
              fieldWidth: Math.floor((content.width - Style.space(24)) / 4)
        ...
        Column {
          ...
          PanelSectionHeader {
            text: "BEDTIME AND BREAKS"
            foreground: Color.foreground
          }

          Text {
            textFormat: Text.PlainText
            text: "A locked period locks the screen (bedtime, dinner). Set school hours in the School + Apps tab."
            width: parent.width
            wrapMode: Text.WordWrap
            color: root.fadeText(0.5)
            font.family: Style.font.family
